...
namespace _1_API.Controllers
{
    ...
    public class NhanViensController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("Creates")]
        public async Task<IActionResult> CreateNhanViens(List<CreateNhanVien> cnv)
        {
            var manv = await _repo.GetAllAsync();
            List<NhanVien> nvs= new List<NhanVien>();
            foreach (var op in cnv)
            {
                NhanVien nv = new NhanVien()
                {
                    Id = Guid.NewGuid(),
                    ...
                    Email = op.Email,
                    MatKhau = op.MatKhau,
                    AnhNhanVien = op.AnhNhanVien,
                    GioiTinh = op.GioiTinh,
                    DiaChi = op.DiaChi,
                    NgaySinh = op.NgaySinh,
                    TrangThai = op.TrangThai,
                    Sdt = op.Sdt,
                };
                nvs.Add(nv);
            }
            try
            {
                var result = await _repo.AddManyAsyn(nvs);
                return Ok(nvs);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> DeleteAll()
        {
            ...
            else
            {
                try
                {
                    await _repo.DeleteManyAsyn(result);
                    return Ok("Xóa thành công");
                }
                catch (Exception ex)
                {
